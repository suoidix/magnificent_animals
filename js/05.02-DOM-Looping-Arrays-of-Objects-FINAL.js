...
const h2 = document.querySelector('h2'); // get h2 for animal name
const p = document.querySelector('p'); // get p tag for description
...
h2.textContent = `${animalsArr[0].name} - ${animalsArr[0].region} - ${animalsArr[0].class}`;
p.innerHTML = animalsArr[0].desc;
...
for(let i = 0; i < animalsArr.length; i++) {
    ...
    let imgFile = animalObj.name.replace(" ", "-").toLowerCase() + ".jpg";
    let imgTag = `<img src="images/${imgFile}" onclick="swapAnimal(${i})">`;
...
function swapAnimal(i) {
    ...
    let animalObj = animalsArr[i];
    h2.textContent = `${animalObj.name} - ${animalObj.class} - ${animalObj.region}`;
    p.innerHTML = animalObj.desc;
    ...
    let iframeTag = `<iframe width="560" height="315" 
    src="https://www.youtube.com/embed/${animalObj.youTube}" 
    title="YouTube video player" frameborder="0" allow="accelerometer; 
    autoplay; clipboard-write; encrypted-media; gyroscope; 
    picture-in-picture; web-share" allowfullscreen></iframe>`;
    videoPlayerDiv.innerHTML = iframeTag;
}
